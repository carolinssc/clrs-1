...
import wandb
run = wandb.init(project="clrs", entity="carosc", config={})

run.define_metric("train_loss", summary="min")
run.define_metric("train_score", summary="max")
run.define_metric("val_score", summary="max")
...
flags.DEFINE_float('sigma', 0.0, 'standard deviation')
...
flags.DEFINE_boolean('load_pretrained', False,
...
def main(unused_argv):
  # Use canonical CLRS-30 samplers.construct_processor
  clrs30_spec = clrs.CLRS30
  logging.info('Using CLRS30 spec: %s', clrs30_spec)
  if FLAGS.sigma !=0: 
    FLAGS.dataset_path = FLAGS.dataset_path +f'_{FLAGS.sigma}'
  sigma = FLAGS.sigma
  ...
  for step in range(FLAGS.train_steps):
    ...
    if step == 0:
      ...
      if FLAGS.load_pretrained:
        logging.info('Restore pretrained model')
        model.init(feedback.features, FLAGS.seed + 1)
        model.restore_model('final_mpnn2.pkl', only_load_processor=True)
      else:
  ...
  model.restore_model(f'mpnn_{sigma}.pkl', only_load_processor=False)
  ...
  text_file = open(f'test_{sigma}', "w")
  text_file.write(str(test_stats))
  text_file.close()
